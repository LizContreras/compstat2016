...
shinyServer(function(input,output,session){
  ...
  output$plot <- renderPlot({
    ...
    if(input$ribbon){p <- p + geom_ribbon(aes(ymin=lower, ymax=upper), fill='blue', alpha=0.5)}
...
cosaMH<- eventReactive(input$Calcular,{
      base<<-Boston[c("crim" ,"black"  ,"rm", "medv")]
      colnames(base)<-c(1,2,3,4)
      y <- base[,c(input$dep)]
      df <<- c(input$checkGroup)
      x <- base[df]
      variables <<- length(df)
...
tabla<<-Comparison
    ...
    output$value <- renderPrint({
      ...
      output$Boston<-renderPlot({
        pairs(Boston[c('crim','black','rm','medv')])      
      })
